...
uint8_t count = 0;
extern tft_t tft;
uint8_t encrypt(uint8_t var, int key_no);
...
//	printf("%u", *p);
	if (*p > 0)
	{
		...
		else							//if not in the same line
		{
			...
			printf(" ");
			tft.setXY(FONT_WIDTH * tem * tft.zoom(), tft.y() - FONT_HEIGHT * tft.zoom());
			uint32_t screen_p[2];
			screen_p[0] = tft.x();
			screen_p[1] = tft.y();
			str[*p] = '\0';
...
void keypad::talk(char *str, uint16_t *p)	//push to talk flag
{
...
uint8_t encrypt(uint8_t var, int key_no)	//encrypt 
{
